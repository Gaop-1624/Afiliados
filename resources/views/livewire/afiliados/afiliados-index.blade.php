    ...
    <x-card2>
        ...
        @if ($afiliados->count())
                     @foreach ($afiliados as $afiliado)
                        <div id="toast-interactive" class="m-1 ml-8 justify-center inline-block mb-2 max-w-xs p-1 text-gray-500 bg-white rounded-lg shadow-2xl dark:bg-gray-800 dark:text-gray-400" role="alert">
                            <div class="flex">
                                ...
                                <div class="ms-1">
                                    <span class="mb-1 text-xs font-bold text-gray-900 dark:text-white">{{$afiliado->pnombre}} {{$afiliado->snombre}} {{$afiliado->papellido}}</span>
                                    <div class="mb-1 text-xs font-italic font-semibold text-blue-600">Cc: {{$afiliado->documento}} &nbsp; {{$afiliado->email}}</div> 
                                     <div class="mb-2"></div> 
                                        ...
                                        <div class="col-span-2">
                                            ...
                                            @if($ultimoContrato)
                                                @if ($ultimoContrato->status == 1)
                                                    <button type="button" class="cursor-pointer text-white bg-blue-600 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-semibold rounded-lg text-xs px-5  text-center me-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 py-1">
                                                        <i class="fas fa-check-circle fa-fw"></i> &nbsp; {{__('Asset')}}
                                                    </button>
                                                @elseif ($ultimoContrato->status == 3)
                                                    <button type="button" class="cursor-pointer text-white bg-violet-600 hover:bg-violet-800 focus:outline-none focus:ring-4 focus:ring-violet-300 font-semibold rounded-lg text-xs px-5  text-center me-2 dark:bg-violet-600 dark:hover:bg-violet-700 dark:focus:ring-violet-800 py-1">
