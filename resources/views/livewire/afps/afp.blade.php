        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li>
                <div class="flex items-center">
                  ...
                  <a href="#" class="ms-1 text-xs font-medium text-blue-400 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">{{__('Pensions (AFP)')}}</a>
                </div>
            </li>
        </ol>
   </nav>
   <x-titulo><i class="fas fa-poll"></i> &nbsp; {{__('List Pensions')}}</x-titulo>
    <x-card2>
          <form method="GET">
                <div class="grid grid-cols-1 gap-1 lg:grid-cols-5 lg:gap-2">
                      ...
                      <div class="justify-right px-10 py-4 mr-4">
                        ...
                        <div class="flex justify-end px-1 py-4 mr-4"> 
                            @can('admin.afp.create')
                                <a href="{{ route('Afps.Afps.Create') }}" class="h-8 px-10 py-1 text-xs font-medium text-center inline-flex items-center text-white bg-blue-700 rounded-lg hover:bg-blue-600 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 whitespace-nowrap" title="{{__("New Pensions")}}" >
                                ...
                                <div class="flex items-start bg-neutral-secondary-soft rounded-xl p-1">
                                    <div class="me-1.5">
                                        <span class="flex items-center gap-2 text-sm font-medium text-heading pb-2">
                                            <span class="mb-1 px-4 text-xs font-bold text-white dark:text-white  bg-cyan-400 w-full"><i class="fas fa-server fa fw" style="color: white"></i> &nbsp; {{$afp->nombre}}</span>
                                        </span>
                                        <h5 class="text-left text-xs text-stone-400 font-semibold border-b-2 rounded-sm shadow-2xl mb-1">Nit: {{$afp->nit}} &nbsp; Cod: {{$afp->codigo}}</h5>
                                        <div class="grid grid-cols-2 gap-4">
                                            <div></div>
                                            <div class="justify-end">
                                                @can('admin.afp.edict')
                                                    <a href="#" wire:click="update({{$afp->id}})" class="inline-flex border px-1 py-1 hover:bg-blue-100 focus:ring-4 focus:outline-none focus:ring-blue-100 dark:bg-blue-200 dark:hover:bg-blue-400 dark:focus:ring-blue-600" title="{{__('Edit')}}">
                                                        <i class="far fa-edit fa-fw fa-xl" style="color: blue;"></i>
                                                    </a>
                                                @endcan
                                                @can('admin.afp.delete')                                    
                                                    <a href="#" wire:click="delete({{$afp->id}})" class="inline-flex border px-1 py-1 hover:bg-red-100 focus:ring-4 focus:outline-none focus:ring-red-100 dark:bg-red-200 dark:hover:bg-red-400 dark:focus:ring-red-600" title="{{__('Delete')}}">
