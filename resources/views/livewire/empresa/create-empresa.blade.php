<div>
   ...
   <x-titulo><i class="fab fa-affiliatetheme" style="color: rgb(166, 168, 166)"></i> &nbsp; {{__('Create Company')}}</x-titulo>
   <x-card>
                ...
                @if ($errors->first('dev')) 
                    <div class="px-1 py-2 border bg-slate-100 h-12">
                        <div class="relative mb-4">
                            ...
                            <label for="dev" class="absolute text-sm text-red-700 duration-300 transform -translate-y-3 scale-75 top-1 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-red-600 peer-focus:dark:text-red-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-1 peer-focus:scale-75 peer-focus:-translate-y-3 start-1 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto font-bold">{{__('Dv')}}</label>
        ...
        <div class="grid grid-cols-1 gap-2 lg:grid-cols-4 lg:gap-2 mb-2  m-2">
            ...
            @if ($errors->first('ciudad_id')) 
                <div class="px-4 py-2 border bg-slate-100">
                    <select wire:model.defer="ciudad_id" wire:ignore class="select2-ciudad block w-full p-2 text-xs text-red-900 border border-red-300 rounded bg-red-50 focus:ring-red-500 focus:border-red-500">
            ...
            @else
                <div  class="px-4 py-2 border bg-slate-100 h-12"  wire:ignore>
                    <select wire:model.defer="ciudad_id"  id="users-select" class="select2-ciudad block w-full p-2 mb-6 text-xs text-gray-900 border border-gray-300  bg-gray-50 focus:ring-blue-500 focus:border-blue-500 h-8">
        ...
        <div class="grid grid-cols-1 gap-2 lg:grid-cols-4 lg:gap-2 mb-2  m-2">
                ...
                @if ($errors->first('caja_id')) 
                    <div class="px-4 py-2 border bg-slate-100">
                        ...
                        <p class="mt-2 text-xs text-red-600">{{$errors->first('caja_id')}} </p>
                    </div>
                @else
        ...
        <div class="flex justify-end border-x-0 mb-4">
            @can('admin.empresa.create') 
                <button wire:click.prevent="create()" class="px-6 py-1 mr-2 h-7 text-xs font-semibold text-center inline-flex items-center text-white bg-blue-700 rounded-sm hover:bg-blue-600 focus:ring-4 focus:outline-none focus:ring-blue-300"><i class="fas fa-save fa-stack fa-lg"></i>{{__('Save')}} </button>
            @endcan
